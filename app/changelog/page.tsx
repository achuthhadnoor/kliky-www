  ...
  const changelogEntries: ChangelogVersion[] = [
    {
      ...
      bullets: [
        {
          type: "acoustics",
          title: "Creamy & 8-Bit Switch Profiles",
          text: "Integrated premium Velvet (Creamy) and Neon (8-bit) high-fidelity mechanical sound packs."
        },
        {
          type: "features",
          title: "Under-3.5ms Web Synthesis",
          text: "Engineered ultra-low-latency Web Audio API synthesizer node with spatial panner nodes."
        },
        {
          type: "features",
          title: "Bento Features Showcase Grid",
          text: "Designed fully modular interactive bento grid featuring live delay testers, key depressors, and click blueprints."
        },
        {
          type: "performance",
          title: "Theme System Integration",
          text: "Shifted color scheme to premium Amethyst Purple and integrated prefers-color-scheme system light/dark compatibility."
        }
      ]
    },
    {
      ...
      bullets: [
        {
          type: "acoustics",
          title: "Custom Sound Folder Support",
          text: "Enabled dragging and dropping custom WAV/MP3 packs directly into local settings directories."
        },
        {
          type: "features",
          title: "Smart Mic Call Dampener",
          text: "Built background mic listener that dampens typing clicks by 65% when voice speech is actively identified."
        },
        {
          type: "features",
          title: "Global Utility Hotkeys",
          text: "Created universal hotkeys (⌥⌘K / Alt+Win+K) to toggle the app's sound engine in the background."
        },
        {
          type: "performance",
          title: "Resource Efficiency",
          text: "Re-engineered native system loops to cap CPU below 1.2% and RAM footprint under 15MB."
        }
      ]
    },
    {
      ...
      bullets: [
        {
          type: "features",
          title: "Universal OS Clients",
          text: "Released native, lightweight desktop utility wrappers for both macOS (Apple Silicon/Intel) and Windows (x64)."
        },
        {
          type: "acoustics",
          title: "Official Switch Packs",
          text: "Preloaded core acoustics with Zenith (crisp tactile), Obsidian (heavy deep thock), and Sapphire (clicky linear)."
        },
        {
          type: "features",
          title: "Spatial Acoustic Synthesizer",
          text: "Configured spatial audio stereo panning based on keyboard coordinates and active pitch/frequency modifiers."
        },
        {
          type: "performance",
          title: "Offline-First Privacy",
          text: "Established absolute offline capabilities to guarantee zero network typing logs."
        }
      ]
    }
  ];

  // Filter version items based on search query and category tab selections
  const filteredEntries = changelogEntries.map((entry) => {
    // Filter bullets by search query and category
    ...
    // Only display versions that have at least one matching bullet, or matches version taglines
        ...
        {!loading && releases.length > 0 && (
          <div className="w-full mb-10 p-4 rounded-2xl glass-panel border border-brand/20 bg-brand/[0.02] flex items-center justify-between text-xs font-sans text-zinc-700 dark:text-zinc-300 relative overflow-hidden">
            ...
            <a 
              ...
              className="font-bold text-brand hover:text-brand-hover shrink-0 underline ml-4 flex items-center gap-1 group select-none cursor-pointer"
            >
              Release Notes
                ...
                <div className="rounded-3xl glass-panel border border-zinc-200/50 dark:border-zinc-800/40 p-6 md:p-8 hover:border-zinc-300 dark:hover:border-zinc-700/80 transition-all duration-300 bg-white/20 dark:bg-zinc-950/20 relative overflow-hidden flex flex-col gap-5">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-zinc-200/40 dark:border-zinc-800/40 pb-4 gap-3">
                    <div>
                      ...
                      <p className="text-[11px] font-bold font-mono text-zinc-400 dark:text-zinc-500 uppercase tracking-widest mt-0.5">
                        {entry.tagline}
                      </p>
                    </div>
                  ...
                  {entry.bullets.length > 0 ? (
                        ...
                        <div key={idx} className="flex items-start">
                          ...
                          <span className={`text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md border shrink-0 mr-4 mt-0.5 select-none font-mono ${
                            bullet.type === "features"
                              ...
                              : bullet.type === "acoustics"
                                ? "bg-indigo-100/50 dark:bg-indigo-950/30 border-indigo-200/30 dark:border-indigo-800/30 text-indigo-500"
                                : "bg-emerald-100/50 dark:bg-emerald-950/30 border-emerald-200/30 dark:border-emerald-800/30 text-emerald-500"
                          }`}>
                            {bullet.type}
                          </span>

                          <div>
                            <h4 className="text-xs font-bold text-zinc-900 dark:text-zinc-100 font-sans tracking-wide">
                              {bullet.title}
                            </h4>
                            <p className="text-xs text-zinc-600 dark:text-zinc-400 font-sans mt-1 leading-relaxed">
                              {bullet.text}
                            </p>
                          </div>
                        </div>
                    ...
                    <p className="text-xs text-zinc-400 font-sans italic text-center py-4">
                      No matching updates under the current filter selection.
                    </p>
                  )}
