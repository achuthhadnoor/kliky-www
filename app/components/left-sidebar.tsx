...
export function LeftSidebar() {
    ...
    try {
      const res = await fetch("/api/waitlist", {
        ...
        body: JSON.stringify({
          email: email.trim(),
          platform,
          source: "Hero Section Download Gate",
        }),
      });
  ...
  return (
    <div className="flex flex-col justify-between h-full py-2 space-y-8">
      ...
      {/* Download Action / Waitlist Section */}
      <div className="space-y-3 pt-6 border-t border-zinc-200 dark:border-zinc-900/60">
        {!isJoined ? (
          <form onSubmit={handleJoinWaitlist} className="space-y-3">
            <div className="space-y-1">
              <label className="text-[10px] font-bold font-mono text-zinc-400 dark:text-zinc-500 uppercase tracking-wider px-0.5">
                Join the Waitlist to Download
              </label>
              <input
                type="email"
                ...
                className="w-full px-4 py-3 rounded-2xl border border-zinc-200 dark:border-zinc-800/80 bg-zinc-50/50 dark:bg-zinc-900/40 text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-brand/35 text-xs font-sans transition-all"
              />
              {error && (
                <p className="text-[10px] text-red-500 font-medium px-1 pt-0.5">
                  {error}
                </p>
              )}
            </div>
            <button
              type="submit"
              disabled={isSubmitting}
              className="glow-on-hover flex w-full items-center justify-center gap-2 rounded-2xl bg-brand py-3.5 text-xs font-bold text-white shadow-xl hover:shadow-brand/20 active:scale-95 transition cursor-pointer select-none border-none outline-none"
            >
          ...
          <div className="space-y-3 animate-in fade-in duration-300">
            <div className="bg-emerald-500/10 dark:bg-emerald-950/20 border border-emerald-500/20 rounded-2xl p-4 text-center">
              ...
              <p className="text-[10px] text-zinc-500 dark:text-zinc-400 mt-1">
                Your download links are unlocked below:
              </p>
            </div>
            ...
            >
              <svg className="w-4.5 h-4.5 fill-current text-zinc-600 dark:text-zinc-300" viewBox="0 0 24 24">
